document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSidebarProfile(index = 0) {
    ...
    if (activePetPic) activePetPic.src = pet.photo || "images/dog.jpg";
    if (activePetName) activePetName.textContent = pet.name || "Unknown";
    if (activePetBreed) activePetBreed.textContent = pet.breed || "Unknown";
    if (activePetWeight) activePetWeight.textContent = pet.weight || "N/A";
  }
  ...
  function renderPets() {
    ...
    document.querySelectorAll(".delete-btn").forEach(btn => {
      btn.addEventListener("click", (e) => {
        const index = e.target.dataset.index;
        pets.splice(index, 1);
        localStorage.setItem(`pets_${currentUser}`, JSON.stringify(pets));
        renderPets();
      });
    });

    populatePetSwitcher();
    updateSidebarProfile(petSwitcher?.value || 0);
  }
      ...
      if (file) {
        ...
        reader.onload = () => {
          ...
          if (editIndex !== undefined) {
            pets[editIndex] = { name, breed, attitudes, photo };
            delete form.dataset.editIndex;
          } else {
            pets.push({ name, breed, attitudes, photo });
          }
        ...
        pets[editIndex].name = name;
        pets[editIndex].breed = breed;
        pets[editIndex].attitudes = attitudes;
        localStorage.setItem(`pets_${currentUser}`, JSON.stringify(pets));
        renderPets();
        form.reset();
        delete form.dataset.editIndex;
